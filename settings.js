...
// Moved here from exercises.js -- adding a new exercise is app-wide data
// management (same category as Import/Export Data right above it), not
// something that should only be reachable while in the middle of logging
// a workout. ensureAIConfig/generateExercisesWithAI (functions.js) do the
// actual config-collection and network call; this file just drives the
// two-step UI (names+hint -> preview list) and merges whichever confirmed
// results the user checks once they hit Save, never before. One call
// generates every name typed in at once -- entered one per line -- rather
...
function showExercisePreviewDialog(results){
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    alert(`${names} added -- find ${selected.length > 1 ? "them" : "it"} next time you add exercises to a workout.`);
  });
